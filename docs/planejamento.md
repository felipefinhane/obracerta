...
- Confirmado: implementação completa da fundação técnica (`.scratch/fundacao-tecnica/`) — schema SQL + RLS de todas as tabelas do MVP, storage via R2 (Route Handler de URL assinada) e pipeline de extração (Edge Function + webhook via `pg_net`), testados de ponta a ponta contra o projeto Supabase e o deploy Vercel hospedados de verdade (não só simulação local), com o `recibo_exemplo.jpg` extraindo dados reais. Detecção de QR code/chave NF-e não implementada (sem imagem de teste real com QR, fica como TODO). Autorização do Route Handler de storage revisada durante a implementação: em vez de RPC com `obra_id` vindo do cliente (risco de "confused deputy"), busca a linha via client Supabase da sessão do usuário — a própria RLS de leitura já prova o acesso (ADR 0001/0003 atualizados).
- Confirmado: login/cadastro do MVP é **email + senha** — mais simples e não depende de entrega de email funcionar bem (magic link, considerado, fica como opção futura sem remover a senha — ver `mvp.md` seção 2).
